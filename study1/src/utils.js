...
export function exportData(data) {
  ...
  return {
    ...
    total_threes_1: totalThrees_1,
    total_threes_2: totalThrees_2,
    total_bonus_1: totalBonus_1,
    total_bonus_2: totalBonus_2,
    gender: gender,
    age: age,
    comment: suggest,
...
export function makeMultipliers(condition, pM) {
  ...
  if (condition != 'inverse streak') {
    while (geomArray_sum != targetSum || geomArray_max > max_strk || geomArray_max < minMax_strk) {
      geomArray = [];
    ...
    for (let i = 0; i < numTrials; i++) {
      if (i == 0) {
        ...
        outcomeArray.push(...Array(geomArray[i]).fill(-1));
      }
      outcomeArray.push(1);   
    };
